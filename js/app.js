...
$(document).on('turbolinks:load', function() {
  ...
  $('[data-quantity="plus"]').click(function(e){
      ...
      if (!isNaN(currentVal)) {
          ...
          $('b#quantity').text(currentVal + 1);
      } else {
          $('#'+fieldName).val(1);
          $('b#quantity').text(1);
      }
      if ($('h2.item-price').attr('data-price') > 0) {
        total_price = ($('h2.item-price').attr('data-price') * $('b#quantity').text()).toFixed(3);
  ...
  // option choices checkbox validation
  $('[data-type="checkbox"]').click(function(e) {
    const item = document.querySelector("h2.item-price");
    option = $(this).attr('data-option');
    choice = $(this).attr('data-choice');
    min = $('#'+option).attr('data-min');
    max = $('#'+option).attr('data-max');
    price = $('#'+choice).attr('data-price');
    price_with_currency = Number(price).toFixed(3) + ' ' + $('h2.item-price').attr('data-currency');
    checked = $('#'+option+' input:checked');

    if (min == 0) {
      if (e.currentTarget.checked) {
        price = Number(price) + Number(item.dataset.price);
        price_with_currency = Number(price).toFixed(3) + ' ' + $('h2.item-price').attr('data-currency');
      } else {
        price = Number(item.dataset.price) - Number(price);
        price_with_currency = Number(price).toFixed(3) + ' ' + $('h2.item-price').attr('data-currency');
      }
    }
    ...
    // 1. max one and checked 2; clear all selection and allow action
    // 2. max more than one
    if (max == 1 && checked.length == 2) {
      // uncheck all
      checked.prop('checked', false);
      // check the checkbox that the event triggered on
      e.currentTarget.checked = true;
      if (price > 0) {
        $('.item-price').text(price_with_currency);
        $('#total-price').text((price * $('b#quantity').text()).toFixed(3));
      }
    } else {
      // disregard action if max exceeded
      if (price > 0) {
        $('.item-price').text(price_with_currency);
        $('#total-price').text((price * $('b#quantity').text()).toFixed(3));
      }
...
function closeNav() {
  ...
  document.getElementById("nav-close").style.bottom = "-35px"
}
